...
def generate_patterns(num_patterns=1000, group_size=10):
    ...
    data_a = [random.randint(INT32_MIN, INT32_MAX) for _ in range(num_patterns)]

    data_b = [random.randint(INT32_MIN, INT32_MAX) for _ in range(num_patterns)]
    ...
    for i in range(0, num_patterns, group_size):
        ...
        mac_sum = sum(a * b for a, b in zip(chunk_a, chunk_b))
    ...
    with open("golden.hex", "w") as fg:
        for val in golden_results:
            ...
            fg.write(f"{unsigned_val:017x}\n")

    # 6. 印出檔案資訊
    print(f"已成功生成 input_a.hex 與 input_b.hex (各 {num_patterns} 筆)")

    print(f"已成功生成 golden.hex ({len(golden_results)} 筆)")
...
if __name__ == "__main__":
    generate_patterns(num_patterns=1000, group_size=10)
